		...
		TTradeOrderFrame3Output *pOut)
{
	if (m_Verbose) {
		cout << m_pid << " <<< TOF3" << endl <<
				...
				m_pid << " -- st_submitted_id: " << pIn->st_submitted_id << endl <<
				...
				m_pid << " -- requested_price: " << pIn->requested_price << endl <<
	...
	if (m_Verbose) {
		cout << m_pid << " - Trade Order Frame 3 (output)" << endl <<
				...
				m_pid << " -- charge_amount: " << pOut->charge_amount <<
						endl <<
				...
				m_pid << " -- type_is_market: " << pOut->type_is_market <<
						endl <<
